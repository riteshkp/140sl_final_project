...
## 1. Load Data (TODO: Change to your specific ticker symbol.)
```{r}
library(tidyverse)
library(tidyquant)  
...
## 2. Pick buy and sell dates (TODO: Change to your specific dates and actions)
```{r}
head(TSLA)
...
## 3. The backtesting algorithm. This prints out each step so you can see what exactly is going on. Nothing to do here :)
```{r}
...
## 4. Run algorithm. (TODO: Replace TSLA with your specific stock ticker)
```{r}
backTest(TSLA, df)
```
